...
export function LessonView({ courseId, lessonId, onBack, onOpenLesson }: LessonViewProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const l = allLessons.find((les) => les.id === lessonId) ?? null;
      setLesson(l);
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="bg-cream-50 rounded-xl2 border border-cream-200 p-6 md:p-8 mb-8 min-h-[300px]">
        ...
        {activeMode === 'eli10' && (
          <div className="reading-text animate-fade-in" dangerouslySetInnerHTML={{ __html: lesson.eli10 }} />
        )}
        {activeMode === 'flashcards' && (
          <FlashcardsMode lesson={lesson} courseId={courseId} />
        )}
        {activeMode === 'quiz' && (
          <QuizMode lesson={lesson} courseId={courseId} onComplete={handleMarkComplete} />
        )}
        {activeMode === 'practice' && (
          <div className="reading-text animate-fade-in" dangerouslySetInnerHTML={{ __html: lesson.practice }} />
        )}
      </div>
        ...
        {prevLesson ? (
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm font-medium text-ink-600 group-hover:text-terracotta-600 transition-colors">
                {prevLesson.title}
              </p>
            </div>
          </button>
      ...
      {isFlipped && (
        <div className="flex items-center justify-center gap-3 animate-fade-in">
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 rounded-xl bg-brick-50 text-brick-500 border border-brick-100 hover:bg-brick-100 font-medium text-sm transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-2 rounded-xl bg-sage-100 text-sage-600 border border-sage-200 hover:bg-sage-200 font-medium text-sm transition-colors"
          >
  ...
  if (finished) {
    ...
    return (
      <div className="text-center py-6 animate-fade-in">
        ...
        <h3 className="font-serif text-2xl text-ink-700 mb-1">
          {isPassing ? 'Well done!' : 'Keep studying'}
        </h3>
        <p className="text-sm text-warmgray-500 mb-4">
          You scored <span className="font-semibold text-ink-600">{score}</span> out of{' '}
          <span className="font-semibold text-ink-600">{questions.length}</span> ({percent}%)
        </p>
        <button
          onClick={handleRestart}
          className="flex items-center gap-2 mx-auto px-5 py-2.5 rounded-xl bg-cream-100 border border-cream-200 text-ink-600 hover:bg-cream-200 font-medium text-sm transition-colors"
        >
    ...
    <div className="animate-fade-in">
        ...
        {questions.map((_, i) => (
          <div
            ...
            className={`h-1 flex-1 rounded-full ${
              i < currentIdx ? (answers[i] ? 'bg-sage-300' : 'bg-brick-200') : i === currentIdx ? 'bg-terracotta-400' : 'bg-cream-300'
            }`}
          />
        ))}
      </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${
                showCorrect
                  ...
                  : showWrong
                    ? 'bg-brick-50 border-brick-200 text-brick-600'
            ...
            >
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0 ${
                ...
                {showCorrect ? <Check className="w-3.5 h-3.5" strokeWidth={2.5} /> : showWrong ? <X className="w-3.5 h-3.5" strokeWidth={2.5} /> : String.fromCharCode(65 + idx)}
              </div>
      ...
      {showResult && (
        <div className="mt-4 animate-fade-in">
          ...
          <button
            onClick={handleNext}
            className="mt-4 w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-terracotta-500 text-cream-50 hover:bg-terracotta-600 font-medium text-sm transition-colors"
          >
